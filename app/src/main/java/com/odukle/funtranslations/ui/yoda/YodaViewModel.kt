...
private const val TAG = "YodaViewModel"
class YodaViewModel : ViewModel() {
    ...
    fun translate(text: String) {
        ...
        if (ma != null) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ma.runOnUiThread {
                    ma.progress_bar_yoda.visibility = View.VISIBLE
                }
                try {
                    val url = URL("https://funtranslations.com/yoda")
                ...
                ma.runOnUiThread {
                    ma.progress_bar_yoda?.visibility = View.GONE
                }
            }
        }
    }
...
//    }
    ...
    fun generateRandomQuote() {
        CoroutineScope(Dispatchers.IO).launch {
            ma?.runOnUiThread {
                yoda?.original_text_yoda?.setText("")
                yoda?.original_text_yoda?.hint = "wait, cooking random text..."
            }
            try {
                val generate =
                ...
                ma?.runOnUiThread {
                    yoda?.original_text_yoda?.hint = ""
                    yoda?.original_text_yoda?.setText(quote.textContent)
                }
            } catch (e: Exception) {
                Log.d(TAG, e.stackTraceToString())
                ma?.runOnUiThread {

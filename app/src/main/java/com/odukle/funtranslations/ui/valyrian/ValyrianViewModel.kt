...
private const val TAG = "ValyrianViewModel"
class ValyrianViewModel : ViewModel() {
    ...
    fun translate(text: String) {
        ...
        if (ma != null) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ma.runOnUiThread {
                    ma.progress_bar_valyrian.visibility = View.VISIBLE
                }
                try {
                    val url = URL("https://funtranslations.com/valyrian")
                ...
                ma.runOnUiThread {
                    ma.progress_bar_valyrian?.visibility = View.GONE
                }
            }
        }
    }
...
//    }
    ...
    fun generateRandomQuote() {
        CoroutineScope(Dispatchers.IO).launch {
            ma?.runOnUiThread {
                valy?.original_text_valyrian?.setText("")
                valy?.original_text_valyrian?.hint = "wait, cooking random text..."
            }
            try {
                val generate =
                ...
                ma?.runOnUiThread {
                    valy?.original_text_valyrian?.hint = ""
                    valy?.original_text_valyrian?.setText(quote.textContent)
                }
            } catch (e: Exception) {
                Log.d(TAG, e.stackTraceToString())
                ma?.runOnUiThread {

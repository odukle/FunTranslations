...
private const val TAG = "MinionViewModel"
    ...
    fun translate(text: String) {
        ...
        if (ma != null) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ma.runOnUiThread {
                    ma.progress_bar_minion.visibility = View.VISIBLE
                }

                try {
                    val url = URL("https://funtranslations.com/minion")
                ...
                ma.runOnUiThread {
                    ma.progress_bar_minion?.visibility = View.GONE
                }
            }
        }
...
//    }
    ...
    fun generateRandomQuote() {
        CoroutineScope(Dispatchers.IO).launch {
            ma?.runOnUiThread {
                mnn?.original_text_minion?.setText("")
                mnn?.original_text_minion?.hint = "wait, cooking random text..."
            }
            try {
                val generate =
                ...
                ma?.runOnUiThread {
                    mnn?.original_text_minion?.hint = ""
                    mnn?.original_text_minion?.setText(quote.textContent)
                }
            } catch (e: Exception) {
                Log.d(TAG, e.stackTraceToString())
                ma?.runOnUiThread {
